...
namespace GYMappWeb.Service
{
    public class TblUserMemberShipService : ITblUserMemberShip
    {
        ...
        public TblUserMemberShipService(GYMappWebContext context, ILogging logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<PagedResult<GetWithPaginationTblUserMemberShipViewModel>> GetAllUserMembershipsAsync(UserParameters userParameters, int gymBranchId)
        {
            try
            {
                ...
                var query = _context.TblUserMemberShips
                    ...
                    .Include(m => m.Off)
                    .Where(m => m.User.GymBranchId == gymBranchId)
                    .AsQueryable();
            ...
            catch (Exception ex)
            {
                await _logger.LogErrorAsync(
                    $"Failed to get all user memberships. GymBranch: {gymBranchId}, Page: {userParameters.PageNumber}, Search: {userParameters.SearchTerm}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(GetAllUserMembershipsAsync)
                );
                throw;
            }
        }
        ...
        public async Task UpdateExpiredMembershipsAsync(int gymBranchId)
        {
            try
            {
                ...
                var expiredMemberships = await _context.TblUserMemberShips
                    .Include(m => m.User)
                    .Where(m => m.EndDate <= today && m.IsActive && m.User.GymBranchId == gymBranchId)
                    .ToListAsync();
            ...
            {
                await _logger.LogErrorAsync(
                    $"Failed to update expired memberships. GymBranch: {gymBranchId}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(UpdateExpiredMembershipsAsync)
                );
                throw;
            }
        }
        ...
        public async Task<bool> AddMembershipAsync(SaveTblUserMemberShipViewModel model, string createdById, int gymBranchId)
        {
            ...
            {
                await _logger.LogErrorAsync(
                    $"Failed to add membership. UserId: {model.UserId}, MembershipTypeId: {model.MemberShipTypesId}, CreatedBy: {createdById}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(AddMembershipAsync)
                );
                throw;
            }
        }
        ...
        public async Task<bool> DeleteMembershipAsync(int id, int gymBranchId)
        {
            ...
            {
                await _logger.LogErrorAsync(
                    $"Failed to delete membership. MembershipId: {id}, GymBranch: {gymBranchId}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(DeleteMembershipAsync)
                );
                throw;
            }
        }
        ...
        public async Task<bool> DeleteFreezesAsync(int userMemberShipId, int gymBranchId)
        {
            ...
            {
                await _logger.LogErrorAsync(
                    $"Failed to delete freezes. UserMembershipId: {userMemberShipId}, GymBranch: {gymBranchId}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(DeleteFreezesAsync)
                );
                throw;
            }
        }
        ...
        public async Task<bool> HasActiveMembershipAsync(int userId, int gymBranchId)
        {
            ...
            {
                await _logger.LogErrorAsync(
                    $"Failed to check active membership. UserId: {userId}, GymBranch: {gymBranchId}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(HasActiveMembershipAsync)
                );
                throw;
            }
        }
        ...
        public async Task<TblUserMemberShip> GetMembershipByIdAsync(int id, int gymBranchId)
        {
            ...
            {
                await _logger.LogErrorAsync(
                    $"Failed to get membership by ID. MembershipId: {id}, GymBranch: {gymBranchId}",
                    ex,
                    nameof(TblUserMemberShipService),
                    nameof(GetMembershipByIdAsync)
                );
                throw;
            }
        }
